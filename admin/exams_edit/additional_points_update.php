...
if (isset($_POST) AND !empty($_POST))
{
	...
	{ 
		...
		{ 
			...
			$query = "SELECT * FROM `"._TABLE_PREFIX_."add_balls_values` ".$where;
			//echo $query;
			...
			if(mysql_num_rows($rez) > 0)
			{
				...
				///*
				$query="update `"._TABLE_PREFIX_."add_balls_values` SET 
						   value='".$value."'
						   ".$where;
				$cat_update = mysql_query($query);
				if($cat_update) { }
				else {
					$update_error[]=$id_student;
				}
				//*/
			}
			else { 
				//echo "<br>".$id_student." must be inserted";
				///*
				$query = "INSERT INTO `"._TABLE_PREFIX_."add_balls_values`
					VALUES(NULL,
					'".$id_student."',
					'".$_POST['edu_subject']."',
					'".$_POST['edu_year']."',
