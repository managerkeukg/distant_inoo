...
<link rel="stylesheet" href="<?php echo _ROOT_PATH_;?>css/show.css" type="text/css">
...
require_once _FUNCTIONS_PATH_."f_iden_spec_of_student.php";
if(_ID_USER_=="1") {$usertype="2";} // programmer
if(_ID_USER_=="3") {$usertype="3";}
if(_ID_USER_=="4") {$usertype="4";}
if(_ID_USER_=="5") {$usertype="1";} // manager
if(_ID_USER_=="6") {$usertype="1";}
...
if (isset($array_messages_staff) AND !empty($array_messages_staff) AND is_array($array_messages_staff))
{
	...
	foreach ($array_messages_staff as $value) {
		...
		if (($value['active']==2) AND ($value['to']==$usertype)) { $new_array[]=$value['from'];}
...
if (isset($user_array) AND !empty($user_array))
{
	...
	{
		$query = "SELECT COUNT(*) FROM `"._TABLE_PREFIX_."messages_staff` WHERE (`active` = '1' OR `active` = '2')  AND (`to`='".$usertype."' 
			AND `from`='".$id_student."') OR (`from`='".$usertype."' AND `to`='".$id_student."') ORDER BY `date` DESC;" ;
			...
			<div> <img class="avatar" src="<?php echo _COMMON_DATA_PATH_."images/no_avt.jpg"; ?>"> 
				<div class="comment-content  <?php echo $class;?>">
					...
					<?php if(isset($new_array)) {$keys = array_keys($new_array, "$id_student"); ?>
						...
						?></a> 
					<a href="read.php?id=<?php echo $id_student;?>">написать</a>
					</div>
				</div>
			</div>
			<br>
